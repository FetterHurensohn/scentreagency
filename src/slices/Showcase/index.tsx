...
const Showcase = ({ slice }: ShowcaseProps): JSX.Element => {
  // Normalize the iconKey to lower case, or use an empty string if not available
  const iconKey = slice.primary.icon?.toLowerCase() || '';

  // Check if iconKey exists in icons object
  const iconElement = icons[iconKey as keyof typeof icons] || <span>Icon not found</span>;
